...
public class OrganizationGroupAdapter
    implements JpaModel<OrganizationGroupEntity>, OrganizationGroupModel {
  ...
  public Stream<OrganizationGroupModel> getSubGroupsStream() {
    ...
    return closing(
        query.getResultStream().map(organization::getGroupById).filter(Objects::nonNull));
  }
  ...
  public Stream<OrganizationRoleModel> getRoleMappingsStream() {
    return group.getRoleMappings().stream()
        .map(m -> new OrganizationRoleAdapter(session, realm, em, m.getRole(), organization));
  }
  ...
  public OrganizationGroupModel getParent() {
    ...
    OrganizationAdapter adapter =
        new OrganizationAdapter(session, realm, em, group.getOrganization());
    return adapter
        .getGroupsStream()
