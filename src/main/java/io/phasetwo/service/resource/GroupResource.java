...
public class GroupResource extends OrganizationAdminResource {
  ...
  public GroupResource(OrganizationAdminResource parent, OrganizationModel organization, String groupId) {
    super(parent);
  ...
  public Response updateGroup(Group representation) {
    ...
    if (representation.getParentId() != null
        && !representation.getParentId().equals(group.getParentId())) {
      ...
      try {
        ...
        throw new ClientErrorException(
            String.format("Level group named '%s' already exists.", group.getName()),
  ...
  protected OrganizationGroupModel applySecondaryProperties(
      Group representation, OrganizationGroupModel group) {
  ...
  public Response userInGroup(@PathParam("userId") String userId) {
    ...
    if (user != null && group.isMember(user)) {
      ...
      throw new NotFoundException(
          String.format("User %s doesn't have membership in group %s", userId, groupId));
    }
  }
  ...
  public Response hasRole(@PathParam("roleName") String roleName) {
    ...
    if (role.hasRole(group)) {
      ...
      throw new NotFoundException(
          String.format("Group %s doesn't have role %s", groupId, roleName));
    }
  }
  ...
  private OrganizationRoleModel getOrganizationRole(String roleName) {
    ...
    if (role == null) {
      throw new NotFoundException(
          String.format(
              "Role '%s' not found in the organization '%s'", roleName, organization.getName()));
    }
    return role;
  }
