...
public class GroupsResource extends OrganizationAdminResource {
    ...
    protected GroupsResource(RealmModel realm, OrganizationModel organization) {
        super(realm);
    ...
    public GroupResource group(@PathParam("groupId") String groupId) {
      if (auth.hasViewOrgs() || auth.hasOrgViewGroups(organization)) {
        ...
        return setupResource(new GroupResource(realm, organization, groupId));
      } else {
    ...
    public Response createGroup(@Valid Group representation) {
      ...
      try {
        ...
        GroupResource groupResource = setupResource(new GroupResource(realm, organization, child.getId()));
        ...
        throw new ClientErrorException(
                String.format("Level group named '%s' already exists.", groupName), Response.Status.CONFLICT);
      }
    }
}
